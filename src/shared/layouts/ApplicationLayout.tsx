...
export default function ApplicationLayout() {
    ...
    const getPageTitle = (): string => {
        ...
        const path = Object.keys(pageTitles).find(route =>
            location.pathname.includes(route) && route !== '/dashboard'
        );
    ...
    const UserControls = ({ compact = false }) => (
        <>
            ...
            >
                {theme === 'dark' ? (
                    <Sun size={16} className="text-amber-400 hover:text-amber-500 transition-colors" />
                ) : (
                    <Moon size={16} className="text-indigo-600 hover:text-indigo-700 transition-colors" />
                )}
            </Button>
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <Button
                        variant="ghost"
                        size="sm"
                        className={`flex items-center gap-1.5 h-8 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 ${compact ? 'px-2' : ''}`}
                    >
                        <Globe size={16} className="text-gray-600 dark:text-gray-300" />
                        <span className={compact ? "hidden" : "hidden sm:inline"}>{language === "es" ? "Español" : "English"}</span>
                    </Button>
                </DropdownMenuTrigger>
            ...
            {!compact && (
                <div className="hidden sm:flex flex-col text-right">
                    <span className="font-semibold text-[#303972] dark:text-white">Ray</span>
                    <span className="text-xs text-[#A098AE] dark:text-gray-400">Admin</span>
                </div>
            )}
                    ...
                    <div className="flex items-center gap-2 px-4 w-full">
                        ...
                        <Breadcrumb className="w-full">
                            <BreadcrumbList className="flex items-center justify-between w-full">
                                <h1 className="font-bold text-lg sm:text-xl lg:text-2xl text-[#303972] dark:text-gray-100 truncate">{getPageTitle()}</h1>

                                <div className="flex items-center ml-auto">
                                    {!isMobile ? (
                                        <div className={`relative ${isTablet ? 'max-w-[180px]' : 'max-w-md'} w-full mx-2 lg:mx-4 hidden md:block`}>
                                            <Input
                                                placeholder="Buscar..."
