...
export const createHeadCells = (cells: { id: string; numeric?: boolean; disablePadding?: boolean; label: string }[]): HeadCelll[] => {
  return cells.map(({ id, numeric = false, disablePadding = false, label }) => ({
    id,
    numeric,
    disablePadding,
    label,
...
export const getColumnKeys = (data: Array<Record<string, any>>, columnDefinitions?: Record<string, any>) => {
